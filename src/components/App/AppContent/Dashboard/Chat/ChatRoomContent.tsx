    ...
    const mappedMessages = messages.map((message: ChatRoomMessage, index) => {
        return (
            <MessageWrapper key={'wrapper_' + index}>
                ...
                <Message>
                    <MessageAuthor
                        style={{ color: message.authorColor || 'inherit' }}
                    >
                        {message.author}:
                    </MessageAuthor>
...
const ChatContent = () => {
    ...
    const [activeChannelMessages, setActiveChannelMessages] = useState<
        ChatRoomMessage[] | null | 'unauthorized'
    >(null);
    ...
    const textChannelListener = useCallback(
        (reply: MessageEvent) => {
            let parsedReply;
            try {
            ...
            if (
                parsedReply.result &&
                parsedReply.command === 'RPC_getTextChannelContent'
            ) {
            ...
            if (parsedReply.event === 'new_message') {
                ...
                if (activeChannelMessages instanceof Array) {
                    const newActives: ChatRoomMessage[] = [
                        newMessage.message,
    ...
    return (
        <ChatContentWrapper>
            <ChatRoom
                key={activeChannelMessages?.length}
