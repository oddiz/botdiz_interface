...
const AddUser = () => {
    ...
    const [addUserResponse, setAddUserResponse] =
        useState<AddSuperUserReply | null>(null);
    ...
    const handleAddUserSubmit = async () => {
        ...
        const response: AddSuperUserReply | void = await fetch(
            config.botdiz_server + '/addsuperuser',
            {
                method: 'POST',
                headers: {
            ...
            .then((res) => res.json())
            .catch((err) =>
                console.log('error while sending fetch to addsuperuser', err),
            );
        ...
        <AddUserWrapper>
            {addUserResponse && (
                <h3
                    style={{
                        color:
                            addUserResponse.status === 'success'
                                ? 'var(--green)'
                                : 'var(--red)',
                    }}
                >
